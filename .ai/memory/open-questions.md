...
| 006 | assumption | Managed context keeps a real task at or under 50% of an unmanaged run's tokens — claimed, never measured. First data point, M-23 Slice 1 (2026-09-11), covers the session launcher alone: `forgeos brief` 566 tokens against `forgeos prompt` 2,429 (UTF-8 bytes / 4), a 4.3x cut on that one input. Second data point, same day, from one adopted project (three cold-session brief trials): the brief measured ~650 tokens and the fair cut against the `prompt` paste block was about 1.5x — smaller than here, because that project's `Do not` list and paths are longer. Both measure the launcher only. Third data point, 2026-09-12, is whole-session and negative: one large database-implementation session on an adopted project started from a brief within budget and still reached its usage limit — broad specification reads, four migrations, a rehearsal run twice, full validation run more than once. The launcher cut does not bound the session; `.ai/contract/economy.md` §4 (the large-session protocol) is the response and is itself unmeasured. The whole-session comparison is still unmeasured, and no 10x or whole-session figure may be claimed | blueprint maintainer | open | 2026-08-25 | M-17 packaging decision | the efficiency claim on the public README (M-20) | — |
